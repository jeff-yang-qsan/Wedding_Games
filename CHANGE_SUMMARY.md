...
**最新修改**: 2026-01-15 - 實現米黃金色主題配色  
**前次修改**: 2026-01-15 - 實現智能角色識別功能  
**歷史修改**: 2026-01-14 - 從抽籤改為手動輸入

---

## 2026-01-15 更新：米黃金色主題配色

### 🎨 新主題：溫暖米黃金色系
- **背景漸變**: 左上角淺土黃色 → 右下角皮膚色偏粉
- **主要配色**: 金黃色系 (#daa520) 按鈕和界面元素
- **文字配色**: 深金棕色 (#8b5a3c) 提升可讀性
- **完美適配**: 婚禮溫馨氛圍

### 修改的檔案
1. **assets/css/game-styles.css** - 完整主題配色更新
   - body 背景漸變：#f5deb3 → #faf0e6 → #fff8dc → #f4c2c2
   - 所有按鈕配色：primary, secondary, success, danger 
   - 表單、文字、動畫元素配色調整
2. **index-standalone.html** - 同步內嵌樣式更新  
3. **docs/THEME-WHEAT-GOLD.md** - 新增主題配置文檔
4. **docs/README.md** - 更新主題說明

### 配色細節
- **主背景**: linear-gradient(135deg, #f5deb3 0%, #faf0e6 25%, #fff8dc 75%, #f4c2c2 100%)
- **主要按鈕**: #daa520 (Goldenrod) → #cd853f (Peru)
- **文字顏色**: #8b5a3c (Dark Goldenrod)
- **花瓣動畫**: 金黃色系漸變效果

---

## 2026-01-15 更新：智能角色識別

### 🎯 新增功能：URL 參數自動角色判斷
- **主持人模式**: 直接開啟網址（無 join 參數）自動設為主持人
- **參賽者模式**: 網址含 join 參數自動設為參賽者 
- **無需手動選擇** - 省去角色選擇步驟，提升用戶體驗

### 修改的檔案
1. **index.html** - 修改初始化邏輯，根據 URL 參數自動觸發角色選擇事件
2. **index-standalone.html** - 同步實現智能角色識別
3. **assets/js/game-controller.js** - 增強 handleRoleSelection 支援參數預填
4. **assets/js/ui-controller.js** - 修改載入完成邏輯實現自動角色判斷
5. **docs/README.md** - 更新文檔說明新功能
6. **auto-role-test.html** - 新增測試頁面

### URL 參數規格
- **join**: 主持人 Peer ID，存在時自動設為參賽者模式
- **room/roomCode**: 房間代碼，用於自動填入參賽者的房間代碼欄位

### 向後兼容性
- ✅ 原有的手動角色選擇功能完全保留（role-selection 元素仍存在）
- ✅ 舊的分享連結格式完全兼容
- ✅ QR Code 生成已包含正確的 join 參數

---

## 2026-01-14 更新：從抽籤改為手動輸入
