...
### 目前主題：米黃金色系
- **溫暖優雅**：採用淺米黃色到皮膚色的漸變背景
- **金黃點綴**：金黃色系按鈕和界面元素
- **婚禮氛圍**：完美適配婚禮慶祝場合
- 詳細配色方案請參考：[主題配置文檔](THEME-WHEAT-GOLD.md)
...
### 遊戲流程
1. **主持人**: 開啟網頁（系統自動識別為主持人），點擊「建立新遊戲」
2. **參賽者**: 用手機掃描 QR Code 或輸入房間代碼（帶有 join 參數的網址會自動識別為參賽者）
3. **遊戲進行**: 五輪數字抽籤，累計得分
4. **宣布結果**: 自動計算獲勝者，提醒頒發小禮物

### 🆕 智能角色識別
- **主持人模式**: 直接開啟 `http://your-domain.com/index.html`
- **參賽者模式**: 開啟帶有 join 參數的網址 `http://your-domain.com/index.html?join=host123&room=456789`
- **無需手動選擇** - 系統根據網址自動判斷角色，直接進入對應介面
